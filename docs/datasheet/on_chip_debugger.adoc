...
| 23:20 | `nscratch`    | `0010`, number of `dscratch*` CPU registers = 2
...
`dcsr` and the debug program counter `dpc`. Additional general purpose scratch registers for debug mode only
(`dscratch0` and `dscratch1) allows faster execution by having a fast-accessible backup register.
